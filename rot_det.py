...
class vertebral_orientation_detection:
	def __init__(self, volume, segments, active_read_log=os.listdir('Data')):
		...
		self.active_read_log = active_read_log
		...
		active_read_log = [active_read_log[number]]
		...
		for i in active_read_log:
			...
			#print('------------------------------------------------------------------------------------')
			slicer.util.loadVolume(Volume)
			seg = slicer.util.loadSegmentation(Segments)
			...
			for segmentID in stats["SegmentIDs"]:
				...
				#try:
					...
					seg.GetClosedSurfaceRepresentation(segmentID, segmentPolyData)
